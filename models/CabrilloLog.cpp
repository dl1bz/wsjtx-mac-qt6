  ...
  QVariant data(QModelIndex const& model_index, int role) const override {
    ...
    if (model_index.isValid() && model_index.column() == columnCount(model_index) - 1) {
      ...
      if (Qt::DisplayRole == role) {
        value = configuration_->bands()->find(QSqlTableModel::data(index(model_index.row(),
                                              fieldIndex("frequency"))).toULongLong());
      }
    } else {
      ...
      if (Qt::DisplayRole == role) {
        if (model_index.column() == fieldIndex("frequency")) {
          ...
          QLocale locale;
          value = locale.toString(QDateTime::fromMSecsSinceEpoch(value.toULongLong() * 1000ull, Qt::UTC),
                                  locale.dateFormat(QLocale::ShortFormat) + " hh:mm:ss");
        }
      }
    }
    return value;
...
void CabrilloLog::export_qsos(QTextStream& stream) const {
  ...
  auto my_call = m_->configuration_->my_callsign();
    stream << QString {"QSO: %1 DG %2 %3 %4 %5 %6\n"}
           ...
           .arg(QDateTime::fromMSecsSinceEpoch(m_->export_query_.value(when_index).toULongLong() * 1000ull,
                Qt::UTC).toString("yyyy-MM-dd hhmm"))
           .arg(my_call, -12)
