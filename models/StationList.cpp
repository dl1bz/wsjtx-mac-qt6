...
public:
  ...
  QModelIndex first_matching_band(QString const& band_name) const {
    ...
    auto matches = bands_->match(bands_->index(0, 0)
                                 ...
                                 , 1
                                 , Qt::MatchExactly);
    return matches.isEmpty() ? QModelIndex {} :
           matches.first();
  }
...
bool StationList::impl::dropMimeData(QMimeData const * data, Qt::DropAction action, int /* row */, int /* column */,
