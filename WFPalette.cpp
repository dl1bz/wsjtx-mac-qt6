...
Colours load_palette(QString const& file_name) {
  ...
  if (file.open(QIODevice::ReadOnly)) {
    ...
    throw_qstring(QObject::tr("Error opening waterfall palette file \"%1\": %2.").arg(file.fileName()).arg(
                          file.errorString()));
  }
  return colours;
}
  ...
  void export_palette() {
    ...
    if (!file_name.isEmpty()) {
      ...
      if (file.open(QFile::WriteOnly | QFile::Truncate | QFile::Text)) {
        ...
        throw_qstring(QObject::tr("Error writing waterfall palette file \"%1\": %2.").arg(file.fileName()).arg(
                              file.errorString()));
      }
    }
  }
