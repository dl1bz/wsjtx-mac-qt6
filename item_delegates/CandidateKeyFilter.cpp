...
auto candidate_key = sourceModel()->index(candidate_row, m_->referenced_key_column_,
                     candidate_parent).data(m_->referenced_key_role_);
...
if (m_->active_key_.isValid() && candidate_key == m_->active_key_.data(m_->referencing_key_role_)) {
  ...
  return m_->referencing_->match(m_->referencing_->index(0, m_->referencing_key_column_), m_->referencing_key_role_,
                                 candidate_key, 1, Qt::MatchExactly).isEmpty();
}
