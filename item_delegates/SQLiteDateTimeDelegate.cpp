...
void SQLiteDateTimeDelegate::setEditorData(QWidget * editor, QModelIndex const& index) const {
  ...
  if (value.isValid() && !value.isNull()) {
    static_cast<QDateTimeEdit *> (editor)->setDateTime(QDateTime::fromMSecsSinceEpoch(value.toULongLong() * 1000ull,
        Qt::UTC));
  }
}

void SQLiteDateTimeDelegate::setModelData(QWidget * editor, QAbstractItemModel * model,
