...
MainWindow::MainWindow(QString const& settings_filename, QWidget *parent) :
  ...
  // (View → Callsign transparency). Initial check state synced from
  // WideGraph's persisted alpha below.
  ...
  // (View → Callsign font size). Initial check state synced from
  // WideGraph's persisted value below.
  ...
  // Decoded-callsign overlay anchor position — exclusive action group
  // (View → Callsign position). Top = legacy stacking down from the
  // waterfall top; Bottom = stack up from the divider so labels sit in
  // the older / faded waterfall area and don't obscure fresh signals.
  ...
  // Callsign-overlay transparency: same one-way pattern as ticks.
  // None=255 (opaque), Low=220, Medium=200, High=175. Persisted under
  // [WideGraph]/decode_label_alpha via WideGraph::setDecodeLabelAlpha.
  {
  ...
  // Callsign-overlay font size: Small=7, Normal=8 (default), Medium=10,
  // Large=12. Persisted under [WideGraph]/decode_label_font_size via
  // WideGraph::setDecodeLabelFontSize.
  {
    auto* wg = m_wide_graph_window.get();
  ...
  // Callsign-overlay anchor position: Top (legacy) or Bottom (sit above
  // the divider so fresh signals at the top of the waterfall remain
  // visible). Persisted under [WideGraph]/decode_label_position via
  // WideGraph::setDecodeLabelPosition.
  {
    auto* wg = m_wide_graph_window.get();
...
void MainWindow::CreateLiveCQ(QStringList cqliveText) {
  ...
  bandInfo = ui->labFreq->text().split(".", Qt::SkipEmptyParts);
  ...
  for (const QString &item : cqliveText) {
    ...
    thePieces = item.split(" ", Qt::SkipEmptyParts);
    int rxFreq = 0.0;
    if ((thePieces.at(6) == "CQ" || thePieces.at(6) == "QRZ" || thePieces.at(6) == "CQV" ||  thePieces.at(6) == "CQH" ||
         thePieces.at(6) == "QRT") && m_myCall.length() >= 3 && m_myGrid.length() >= 4) {
      try {
        ...
        QMessageBox::critical(this, "Exception",
                              "Exception at line 1116 MainWindow::CreateLiveCQ " + QString::fromStdString(e.what()));
      } catch (...) {
...
void MainWindow::sendLiveCQData(QList<QStringList>decodeList) {
  ...
  for (const QStringList &thePostLine : decodeList) {
    ...
    QString postString =  "skedfreq=" + thePostLine.at(0) + "&rxfreq=" + thePostLine.at(1) + "&rpol=" + thePostLine.at(
                                  2) + "&dt="  +  thePostLine.at(4) + "&dB="  + thePostLine.at(5) + "&msgtype="  +  thePostLine.at(
                                  7) + "&callsign="  +  thePostLine.at(8) + "&grid="  +  thePostLine.at(9) + "&mode="  +  thePostLine.at(
                                  6) + "&utcdatetime="  +  utcdatetimeUTCString + "&spotter="  +  thePostLine.at(12) + "&spottergrid=" +  thePostLine.at(
                                  10)  + "&txpol=" + thePostLine.at(13) + "&apptype=QMAP";
    ...
    try {
      ...
      // Handle standard C++ exceptions
      QMessageBox::critical(this, "Exception",
                            "Exception at line 1165 MainWindow::sendLiveCQData " + QString::fromStdString(e.what()));
    } catch (...) {
      // Handle any other type of exception
...
void MainWindow::handleReply() {
  try {
    ...
    // Handle standard C++ exceptions
    QMessageBox::critical(this, "Exception",
                          "Exception at line 1188 MainWindow::handleReply " + QString::fromStdString(e.what()));
  } catch (...) {
    // Handle any other type of exception
...
void MainWindow::guiUpdate() {
  ...
  if (decodes_.ndecodes > m_fetched) {
    ...
    while (m_fetched < decodes_.ndecodes) {
      ...
      {
        const QStringList cols = QString(t).split(QRegularExpression("\\s+"), Qt::SkipEmptyParts);
        if (cols.size() >= 7) {
          bool ok = false;
          double freq_khz = cols[1].toDouble(&ok);
          if (cols.size() >= 2) {
            bool ok2 = false;
            int ndf = cols[1].toInt(&ok2);
            // ndf is only valid if it is in the range -500..+500
            if (ok2 && std::abs(ndf) <= 500) {
              freq_khz = freq_khz + ndf / 1000.0;
            }
          }
          if (ok) {
