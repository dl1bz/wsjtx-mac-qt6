...
MainWindow::MainWindow(QDir const& temp_directory, bool multiple,
  ...
  // Left-click on a callsign overlay rect populates DX Call and moves
  // the RX bars; TX is intentionally untouched (see slot for rationale).
  connect(m_wideGraph.data(), &WideGraph::callsignClicked,
          this, &MainWindow::overlayCallsignClicked);
          ...
          ui->actionCallsign_overlay_enabled, &QAction::setChecked);
  {
    ...
    case DecodeLabelFontSize::Small:
      ui->actionCallsign_font_small->setChecked(true);
      break;
    case DecodeLabelFontSize::Medium:
      ui->actionCallsign_font_medium->setChecked(true);
      break;
    case DecodeLabelFontSize::Large:
      ui->actionCallsign_font_large->setChecked(true);
      break;
    case DecodeLabelFontSize::Normal:
    default:
      ui->actionCallsign_font_normal->setChecked(true);
      break;
    }
    ...
    // just above the divider so fresh signals at the top of the
    // waterfall remain visible). Persisted via WideGraph as
    // [WideGraph]/DecodeLabelPosition.
    ...
    if (m_wideGraph->decodeLabelPosition() == DecodeLabelPosition::Bottom) {
...
void MainWindow::writeSettings() {
  ...
  m_settings->setValue("DXcallOnly", DXcall_only);
...
void MainWindow::readSettings() {
  ...
  DXcall_only = m_settings->value("DXcallOnly", false).toBool();
  ...
  m_nFT8RXfSens = m_settings->value("NFT8QSORXfreqSensitivity", 3).toInt();
  if (!(m_nFT8RXfSens >= 1 && m_nFT8RXfSens <= 3)) { m_nFT8RXfSens = 3; }
  ...
  m_audioThreadPriority = static_cast<QThread::Priority> (m_settings->value("Audio/ThreadPriority",
                          QThread::TimeCriticalPriority).toInt() % 8);
  m_settings->endGroup();
  m_specOp = m_config.special_op_id();
...
void MainWindow::dataSink(qint64 frames) {
  ...
  if (m_mode == "FT8" && !m_diskData && !(m_multithreadFT8 &&
                                          m_ft8DecoderStart > 1)) { //ft8md Try to call MTD two times when "Very early" has been selected
  ...
  if (bCallDecoder) {
    ...
    if (m_mode == "FT8" and !(m_diskData or (m_multithreadFT8 && m_ft8DecoderStart < 2)) and (m_ihsym == m_earlyDecode or
        m_ihsym == m_earlyDecode2)) { return; }
    if (!m_diskData) {
...
void MainWindow::fastSink(qint64 frames) {
  ...
  if (bmsk144 and (line[0] != 0)) {
    ...
    if (ui->actionHideIgnored->isChecked() or ui->actionHideToday->isChecked() or ui->actionIgnoreIgnored->isChecked() or
        ui->actionIgnoreToday->isChecked()) {
    ...
    if (ui->DX_Call_Button->isChecked() && m_hisCall != "" &&
        text.contains(" " + m_config.my_callsign() + " " + m_hisCall)) {
    ...
    if (m_mode == "MSK144" && wait_and_call && m_specOp != SpecOp::FOX && ui->cbAutoSeq->isChecked() &&
        m_hisCall != "" && (text.contains("CQ " + m_hisCall) or text.contains("CQ DX " + m_hisCall) or
                            text.contains(m_hisCall + " RR73")
    ...
    // DX Call only for MSK144
    if (DXcall_only && !ui->cbBypass->isChecked() && (m_hisCall != "") && (!(text.contains(" " + m_hisCall + " ")))) { filtered = true; }
    ...
    if (!filtered or m_config.filters_for_Wait_and_Pounce_only()) {
      ...
      int ntime = 6;
      if ((m_config.insert_blank() or m_config.alert_Enabled()) && !BlankLineInserted && (text.left(ntime) != m_tBlankLine) &&
          text.left(4).contains(QRegularExpression {"\\d\\d\\d\\d"}) && !m_diskData) {
          ...
          if (m_config.insert_blank()) {
            if (ui->actionUse_Dark_Style->isChecked()) {
              if (m_config.detailed_blank()) {
                if (m_config.DXCC()) {
                  ui->decodedTextBrowser->insertText(("------ " + m_dateTimeSeqStart.toString("yyyy-MM-dd - hh:mm:ss' UTC - '") +
                                                      m_currentBandPeriod + " - " + m_mode + " ------"), "#a2a2a2", "#000000");
                } else {
                  ui->decodedTextBrowser->insertText(("------ " + m_dateTimeSeqStart.toString("yyyy-MM-dd - hh:mm:ss' UTC - '") +
                                                      m_currentBandPeriod + " - " + m_mode), "#a2a2a2", "#000000");
                }
              ...
              if (m_config.detailed_blank()) {
                if (m_config.DXCC()) {
                  ui->decodedTextBrowser->insertLineSpacer("------ " + m_dateTimeSeqStart.toString("yyyy-MM-dd - hh:mm:ss' UTC - '") +
                      m_currentBandPeriod + " - " + m_mode + " ------");
                } else {
                  ui->decodedTextBrowser->insertLineSpacer("------ " + m_dateTimeSeqStart.toString("yyyy-MM-dd - hh:mm:ss' UTC - '") +
                      m_currentBandPeriod + " - " + m_mode);
                }
      ...
      ui->decodedTextBrowser->displayDecodedText(decodedtext, m_config.my_callsign(), m_mode, m_config.DXCC(),
          ...
          ui->cbCQonly->isVisible() && ui->cbCQonly->isChecked() && !DXcall_only && !ui->cbBypass->isChecked(),
    ...
    if (m_config.repeat_Tx() && m_mode == "MSK144" && m_hisCall != "" && text.contains(m_baseCall) &&
        text.contains(m_hisCall + " 73") && m_send_RR73)
    ...
    if (m_config.highlight_orange() or (m_config.highlight_blue()) or ui->actionHighlight_Whitelist_entries->isChecked()) {
      ...
      if (m_config.highlight_orange() && deCall.size() > 2 && (m_config.highlight_orange_callsigns().contains(deCall + ",")
          or m_config.highlight_orange_callsigns().contains(";" + deCall.left(3) + ";") or
          m_config.highlight_orange_callsigns().contains(";" + deCall.left(2) + ";")
          or (m_config.highlight_orange_callsigns().contains(";" + deCall.left(1) + ";") &&
              deCall.left(2).right(1).at(0).isDigit())
          or m_config.highlight_orange_callsigns().contains(";" + deCall.left(1) + "*;")
                                                              ) && !m_config.highlight_orange_callsigns().contains("!" + deCall.left(2) + "!") &&
          !m_config.highlight_orange_callsigns().contains("!" + deCall.left(3) + "!")) {
      ...
      if (m_config.highlight_blue() && deCall.size() > 2 && (m_config.highlight_blue_callsigns().contains(deCall + ",")
          or m_config.highlight_blue_callsigns().contains(";" + deCall.left(3) + ";") or
          m_config.highlight_blue_callsigns().contains(";" + deCall.left(2) + ";")
          or (m_config.highlight_blue_callsigns().contains(";" + deCall.left(1) + ";") && deCall.left(2).right(1).at(0).isDigit())
          or m_config.highlight_blue_callsigns().contains(";" + deCall.left(1) + "*;")
                                                            ) && !m_config.highlight_blue_callsigns().contains("!" + deCall.left(2) + "!") &&
          !m_config.highlight_blue_callsigns().contains("!" + deCall.left(3) + "!")) {
    ...
    if (ui->actionHighlightB4->isChecked() or ui->actionHighlightToday->isChecked() or
        ui->actionHighlightIgnored->isChecked()
    ...
    if (!pounce && (m_config.highlight_DXcall() or m_config.alert_Enabled()) && (m_hisCall != "") &&
        ((text.contains(QRegularExpression {"(\\w+) " + m_hisCall}))
        ...
        if (element.contains(QString("."))) {
          ...
          if (the_message.length() > 0) {
            ...
            if (match.hasMatch()) {
              ...
              if (m_config.alert_Enabled() && m_config.alert_QSYmessage() && (the_line.contains(qCall) or
                  the_line.contains(qDXCall))) { alertQSYmessage(); }
            }
          }
        }
      }
    } else if (((the_line.mid(22).contains(qDXCall + QString(".") + "OKQSY") ||
                 the_line.mid(22).contains(qDXCall + QString(".") + "NOQSY"))) && ui->actionEnable_QSY_Popups->isChecked()) {
      QString yesOrNo = " ";
...
void MainWindow::handleVerifyMsg(int status, QDateTime ts, QString callsign, QString code, unsigned int hz,
...
QString MainWindow::userAgent() {
  ...
  //
  QString platform = "(" + QSysInfo::prettyProductName() + "; " + QSysInfo::productType() + " " +
...
void MainWindow::createStatusBar() {                         //createStatusBar
  ...
  last_tx_label.setMinimumSize(QSize {120, 18});
  ...
  band_hopping_label.setMinimumSize(QSize {120, 18});
...
void MainWindow::on_actionQSYMessage_Creator_triggered() {
  if (!m_QSYMessageCreatorWidget) {
    ...
    connect(m_QSYMessageCreatorWidget.data(), &QSYMessageCreator::sendQSYMessageCreatorStatus, this,
            &MainWindow::setQSYMessageCreatorStatus);
  }
...
void MainWindow::on_contest_log_action_triggered() {
  ...
  connect(m_logBook.contest_log(), &CabrilloLog::qso_count_changed, m_contestLogWindow.data(),
          &CabrilloLogWindow::set_nQSO);
  m_contestLogWindow->set_nQSO(m_logBook.contest_log()->n_qso());
}
...
void MainWindow::on_actionColors_triggered() {
  if (!m_colorHighlighting) {
    ...
    connect(&m_config, &Configuration::decode_highlighting_changed, m_colorHighlighting.data(),
            &ColorHighlighting::set_items);
  }
...
void MainWindow::on_actionActiveStations_triggered() {
  ...
  connect(m_ActiveStationsWidget.data(), SIGNAL(queueActiveWindowHound(QString)), this,
          SLOT(queueActiveWindowHound2(QString)), static_cast<Qt::ConnectionType>(Qt::UniqueConnection));
...
void MainWindow::read_wav_file(QString const& fname) {
  ...
  if (m_config.insert_blank() && !(ui->actionInclude_averaging->isVisible() &&
                                   ui->actionInclude_averaging->isChecked())) {   // insert blank line
    if (!fname.isEmpty()) {
      if (ui->actionUse_Dark_Style->isChecked()) {
...
void MainWindow::overlayCallsignClicked(QString call, int freq_hz, bool decoded_even_period, QString grid) {
  // Left-click on a callsign rect in the waterfall overlay. Populate
  // DX Call, move the RX frequency cursor to that signal, and flip Tx
  // Even/1st to the OPPOSITE of the decoded station's period so the
  // reply lands in the right slot. We deliberately do NOT touch
  // TxFreqSpinBox — that matches plain-click behaviour elsewhere in
  // the waterfall and means Hold Tx Freq is automatically respected.
  if (call.size() < 3) { return; }
  if (m_specOp == SpecOp::FOX || m_specOp == SpecOp::HOUND) { return; }
  ui->dxCallEntry->setText(call);
  if (!grid.isEmpty()) { ui->dxGridEntry->setText(grid); }
  ui->RxFreqSpinBox->setValue(freq_hz);
  ui->txFirstCheckBox->setChecked(!decoded_even_period);
  on_dxCallEntry_editingFinished();
  // Prevent double-clicking from searching for the callsigns on QRZ.com.
  // Allow starting a QSO under certain conditions by double-clicking.
  if (!not_now) {
    on_lookupButton_clicked();
    not_now = true;
    QTimer::singleShot(500, [ = ] {not_now = false;});
    on_genStdMsgsPushButton_clicked();
  } else {
    if (m_config.quick_call() && ui->cbHoldTxFreq->isChecked() && !m_auto) { ui->autoButton->click(); }
  }
}
...
void MainWindow::decode() {                                     //decode()
  ...
  if (m_mode == "FT8" && m_multithreadFT8) {
    ...
    if (m_houndMode) { dec_data.params.nft8rxfsens = 1; }
    else { dec_data.params.nft8rxfsens = m_nFT8RXfSens; }
    ...
    dec_data.params.ltxing = (m_auto &&
                              (QDateTime::currentMSecsSinceEpoch() - m_mslastTX) < 26000) ? 1 : 0; // ft8mdwas ( m_enableTx and jtdxTime etc.
    dec_data.params.lhideft8dupes = ui->actionHide_FT8_dupe_messages->isChecked() ? 1 :
                                    0; //ft8md ui->actionHide_FT8_dupe_messages->isChecked() ? 1 : 0;
...
void MainWindow::decodeDone () {
  ...
  if (m_mode != "FT8" or dec_data.params.nzhsym == 50 or (m_mode == "FT8" and m_multithreadFT8 and
      (m_hsymStop == dec_data.params.nzhsym))) { m_nDecodes = 0; } //ft8md
...
void MainWindow::queueActiveWindowHound2(QString line) {
  ...
  if (m_mode == "FT8" and m_specOp == SpecOp::FOX) {
    ...
    if  ((w.size() > 7) &&
      ...
      QString db = w[1];
      int db_i = w[1].toInt();
      db = (db_i >= 0 ? "+" : "") + QStringLiteral("%1").arg(db_i, (db_i >= 0 ? 2 : 3), 10,
           QLatin1Char('0')); // +00, -01 etc.
      // houndcall rpt grid
...
void MainWindow::readFromStdout() {                           //readFromStdout
  ...
  while (proc_jt9.canReadLine()) {
    ...
    if (m_mode == "FT8" and m_specOp == SpecOp::FOX and
        m_ActiveStationsWidget != NULL) { // see if we should add this to ActiveStations window
    ...
    if ((!((no_a7_decodes && line_read.contains("a7") && !m_diskData) or (line_read.contains("a7") &&
        ...
        and (!((SpecOp::NONE == m_specOp or (m_multithreadFT8 && m_ft8DecoderStart < 2)) &&
               ((m_mode == "FT8" && ((m_multithreadFT8 && m_ft8DecoderStart < 2) or m_freqNominal > 45000000) &&
                 earlyDecodes.contains(line_read.mid(23, 19)))  // MTD or a7/a8 dupes
        ...
        and (!(SpecOp::NONE == m_specOp && ui->actionReduce_false_decodes->isChecked() && m_mode == "FT8" &&
               ((((message0.contains("/P ") && message0.contains(" R "))                   // /P and R
                ...
                or ((message0.contains("/R ") or message0.contains(" R ")) &&
                    decodedtext.snr() < -18)  // for /R or contest calls SNRmin = -18
      ...
      if (m_mode == "FT8" && !m_multithreadFT8 && ui->actionReduce_false_decodes->isChecked() && decodedtext.snr() < -20) {
        ...
        decodedtext.deCallAndGrid(/*out*/deCall, deGrid);
        QStringList word;
        word = decodedtext.string().mid(24).replace("<", "").replace(">", "").replace("/P", "").replace("/R",
               "").replace("/QRP", "").replace("/5W", "").split(" ", SkipEmptyParts);
        // check file ALLCALL7.TXT
        if (deCall != "TNX" && deCall != "73" && deCall != "GL" && deCall != "HNY" && deCall != "TU" &&
            !deCall.left(4).contains("/")
        ...
        if (notInALLCALL7) {
          ...
          if (word[0] != "CQ" && word[0] != "TNX" && word[0] != "73 " && word[0] != "HNY" && word[0] != "QSY" &&
              word[0] != "PSE" &&
      ...
      if (!bAvgMsg) {
        if (m_mode == "FT8" and SpecOp::FOX == m_specOp) {
          ...
          if ((m_mode == "FT4" or m_mode == "FT2") && SpecOp::NA_VHF == m_specOp && m_config.NCCC_Sprint() && m_hisCall != "" &&
              m_hisGrid != "" &&
          ...
          if ((m_mode.startsWith ("FT") or m_mode == "Q65" or m_mode == "FST4" or m_mode == "JT65" or m_mode == "JT9" or
               m_mode == "JT4") && m_hisCall != ""
          ...
          // Wait & Call
          if (wait_and_call && (m_mode.startsWith ("FT") or m_mode == "Q65" or m_mode == "FST4" or m_mode == "JT65" or
                                m_mode == "JT9" or m_mode == "JT4") &&
          ...
          if ((m_config.insert_blank () or m_config.alert_Enabled()) && (line_read.left(ntime) != m_tBlankLine) &&
              message0.left(4).contains(QRegularExpression {"\\d\\d\\d\\d"}) && !m_diskData) {
            ui->decodedTextBrowser->new_period ();
            if (m_specOp == SpecOp::FOX and m_ActiveStationsWidget != NULL &&
                m_config.insert_blank ()) { // clear the ActiveStations window
...
#ifndef FOX_OTP
            if (m_specOp == SpecOp::HOUND && m_config.superFox() && (decodedtext0.mid(4, 2).contains("00") or
                decodedtext0.mid(4, 2).contains("30"))) { verified = false; }
#endif
          }
          if ((!verified && ui->labDXped->isVisible()) or !ui->labDXped->text().contains("Hound")) {
            ui->labDXped->setStyleSheet("QLabel {background-color: red; color: white;}");
      ...
      if (m_mode.startsWith ("FT") or m_mode == "FST4" or m_mode == "Q65") {
        ...
        if (parts.size() > 6) {
          ...
          if (SpecOp::FOX == m_specOp and for_us and decodedtext.string().contains(QRegularExpression{" R\\W\\d"})) bDisplayRight
              = true;
      ...
      if ((m_mode == "JT4" or m_mode == "Q65" or m_mode == "JT65") and decodedtext.string().contains(m_baseCall) &&
          ui->actionInclude_averaging->isVisible() && !ui->actionInclude_averaging->isChecked()) { bDisplayRight = true; }
      if (m_mode.startsWith ("FT") and SpecOp::FOX != m_specOp &&
          decodedtext0.string().replace("<", "").replace(">", "").contains(m_baseCall + " " + m_hisCall)) { bDisplayRight = true; } // really all messages for us
      // AutoSeq for JT65/JT4 short messages
      if ((m_mode == "JT65" or m_mode == "JT4") && m_auto && m_config.enable_VHF_features() && ui->cbShMsgs->isChecked() &&
          ui->cbAutoSeq->isChecked () && (abs(audioFreq - m_wideGraph->rxFreq()) <= 15)) {
      ...
      if (bDisplayRight && !block_right_display) {
        ...
        if (!pounce && (!m_bBestSPArmed or (m_mode != "FT4" and m_mode != "FT2"))) {
          ...
          if (m_config.insert_blank () && SpecOp::FOX != m_specOp && m_band_changed && (m_currentBandPeriod == m_currentBand)) {
            if (ui->actionUse_Dark_Style->isChecked()) {
              ui->decodedTextBrowser2->insertText(("------------------- " + m_currentBandPeriod + " -----------------"), "#a2a2a2",
                                                  "#000000");
            } else {
...
void MainWindow::guiUpdate() {
  ...
  if ((g_iptt == 1 && m_iptt0 == 0) || m_restart) {
    ...
    bool b = (m_mode.startsWith ("FT") or "Q65" == m_mode or "JT65" == m_mode or "JT9" == m_mode) and
  ...
  if (nsec != m_sec0) {
    ...
    if (m_mode == "MSK144") {
      QDateTime now = QDateTime::currentDateTimeUtc();
      int s = now.time().toString("ss").toInt();
      if ((m_TRperiod == 5 && (s == 0 || s == 5 || s == 10 || s == 15 || s == 20 || s == 25 || s == 30 || s == 35 ||
                               s == 40 || s == 45 || s == 50 || s == 55))
      ...
      if (m_transmitting) {
        if (m_useDarkStyle) {
          progressBar.setStyleSheet(
                  QString("QProgressBar {color: #ffffff; text-align: center;} QProgressBar::chunk {background-color: #ff0000;}"));  // #ff6464;
          progressBar.setFormat ("%v/%m");
        } else {
#ifdef WIN32
          if (m_TRperiod > 99) {
            progressBar.setStyleSheet(
                    QString("QProgressBar {color: #000000; text-align: right; margin-right: 4em;} QProgressBar::chunk {background-color: #ff0000;}")); // #ff4141
            progressBar.setFormat ("%v/%m ");
          } else {
            progressBar.setStyleSheet(
                    QString("QProgressBar {color: #000000; text-align: right; margin-right: 3em;} QProgressBar::chunk {background-color: #ff0000;}")); // #ff4141
            progressBar.setFormat ("%v/%m  ");
          }
#else
          progressBar.setStyleSheet(
                  QString("QProgressBar {color: #000000; text-align: center;} QProgressBar::chunk {background-color: #ff4141;}"));  // #ff6464;
          progressBar.setFormat ("%v/%m");
#endif
        }
      } else {
        ...
        if (m_useDarkStyle) {
          progressBar.setStyleSheet(
                  QString("QProgressBar {color: #ffffff; text-align: center;} QProgressBar::chunk {background-color: #1464A0;}"));   // for macOS
        } else {
          progressBar.setStyleSheet(
                  QString("QProgressBar {color: #000000; text-align: center;} QProgressBar::chunk {background-color: #96C9F6;}"));   // for macOS
        }
#else
        progressBar.setStyleSheet("");
#endif
    ...
    if (m_transmitting) {
      ...
      if (!m_tx_watchdog) {
        ...
        if (DXcall_only && !ui->cbBypass->isChecked() && SpecOp::HOUND != m_specOp && SpecOp::FOX != m_specOp) {
          tx_status_label.setStyleSheet ("QLabel{color: #000000; background-color: #00ffff}");
          t = " Receiving, Filters On ";
        }
...
void MainWindow::doubleClickOnCall(Qt::KeyboardModifiers modifiers) {
  ...
  if (SpecOp::HOUND == m_specOp && (message.string().mid(4, 2).contains("15") or
                                    message.string().mid(4, 2).contains("45"))) { return; }  // ignore stations calling in the wrong time slot
  ...
  if (ui->txFirstCheckBox->isVisible() && !ui->txFirstCheckBox->isEnabled() && (
    ...
    if (m_mode == "MSK144" && message.frequencyOffset() > 0 && ((modifiers & Qt::ControlModifier) &&
  ...
  // Set Tx freq for FT2, FT4, FT8 and FST4 when double-clicking on a message and Hold Tx Freq is not checked
  int frequency = message.frequencyOffset();
  if ((m_mode.startsWith("FT") or m_mode == "FST4") and !ui->cbHoldTxFreq->isChecked ()) { ui->TxFreqSpinBox->setValue(frequency); }
}
...
void MainWindow::processMessage (DecodedText const& message, Qt::KeyboardModifiers modifiers) {
  ...
  if (m_bDoubleClicked && message.clean_string().remove("<").remove(">").contains((" " + m_baseCall + " "))
      && message.clean_string().remove("<").remove(">").contains(" " + hiscall + " ") &&
      message.clean_string().mid(22).contains(" 73")) { return; }
        ...
        if (("RRR" == word_3
          ...
          if (SpecOp::RTTY == m_specOp and m_nextCall != "") {
            ...
            if (false) {            // Always Send 73 after receiving RRR or RR73, even in contest mode.
              ...
              if ((m_mode == "MSK144" or (m_mode == "Q65" && m_config.repeat_Tx())) &&
                  !m_send_RR73) { // ensure that 73 is sent when using RRR
                ui->txrb5->click();
  ...
  if (s1 != s2 and !message.isTX()) {
    ...
    if (m_mode == "MSK144" && m_config.insert_blank () && m_band_changed && (m_currentBandPeriod == m_currentBand) &&
        !BlankLineInserted) {
      if (ui->actionUse_Dark_Style->isChecked()) {
        ui->decodedTextBrowser2->insertText(("------------------- " + m_currentBandPeriod + " -----------------"), "#a2a2a2",
                                            "#000000");
  ...
  if (Radio::is_callsign (hiscall)
    ...
    if (!((s2.contains(" " + m_config.my_callsign() + " ") && s2.mid(22).contains(" 73")) &&
  ...
  if (eme_short_codes) {
    ...
    if (is77BitMode ()) {
      ...
      if (!bHisCall) {
        ...
        if (is77BitMode () && SpecOp::NONE == m_specOp && ((Radio::is_77bit_nonstandard_callsign (my_callsign) &&
            !is_compound) or
      ...
      if (is77BitMode () && SpecOp::NONE == m_specOp && Radio::is_77bit_nonstandard_callsign (my_callsign) &&
          ...
          && !(!Radio::is_77bit_nonstandard_callsign (m_baseCall) && hisCall.contains("/P") &&
               !Radio::is_77bit_nonstandard_callsign (hisBase) && my_callsign.contains("/P"))) {
        ...
        if ((is_compound or my_callsign.contains("/P") or hisCall != hisBase) &&
            Radio::is_77bit_nonstandard_callsign (hisBase)) { t = "<" + hisCall + "> <" + my_callsign + "> "; }
...
void MainWindow::mousePressEvent(QMouseEvent *event) {  // mouse press events
  ...
  if (ui->cbCQonly->hasFocus() && (event->button() & Qt::RightButton)) {       // DX Call only
    if (DXcall_only) {
      DXcall_only = false;
      ...
      QTimer::singleShot (100, [ = ] {not_now = false;});
    }
    ui->cbCQonly->clearFocus();
    check_button_color();
  }
  ...
  if (ui->autoButton->hasFocus() && (event->button() & Qt::RightButton) &&
      ui->respondComboBox->currentText() != "CQ: None") {
...
void MainWindow::on_logQSOButton_clicked() {               //Log QSO button
  ...
  if (m_config.ZZ00() && m_hisGrid == "" && m_specOp != SpecOp::NONE && m_specOp != SpecOp::FOX &&
      m_specOp != SpecOp::HOUND) { m_hisGrid = "ZZ00"; }
  switch (m_specOp) {
  case SpecOp::NA_VHF:
...
void MainWindow::on_actionFT8_triggered() {
  ...
  if (SpecOp::FOX == m_specOp) {
    ...
    if (DXcall_only or ui->cbCQonly->isChecked()) {
      DXcall_only = false;                 // Reset DX Call only
      ui->cbCQonly->setChecked(false);     // Uncheck CQ only
    }
  }
  if (SpecOp::HOUND == m_specOp) {
...
void MainWindow::switch_mode (Mode mode) {
  ...
  if (last_mode != m_mode) {
    no_a7_decodes =
            true;  // Don't allow a7 decodes during the first period because they can be leftovers from the previous mode
...
void MainWindow::on_TxFreqSpinBox_valueChanged(int n) {
  ...
  if (m_mode == "Q65") {
    if (((m_nSubMode == 5 && m_TRperiod == 120.0) || (m_nSubMode == 4 && m_TRperiod == 60.0) || (m_nSubMode == 3 &&
         m_TRperiod == 30.0) ||
...
void MainWindow::on_bandComboBox_currentIndexChanged (int index) {
  ...
  auto const& source_index = frequencies->mapToSource (frequencies->index (index,
                             FrequencyList_v2_101::frequency_column));
...
void MainWindow::handle_transceiver_update (Transceiver::TransceiverState const& s) {
  ...
  if (m_config.PWR_and_SWR()) {
    ...
    if (m_rigState.swr() != s.swr() or m_rigState.alc() != s.alc()) {
      static bool s_alreadyShowingSWRAlert = false;
      if (s.swr() > 0 && m_transmitting) {
        if (s.swr() > 150 or s.alc() > 50) { band_hopping_label.setStyleSheet ("QLabel{color: #000000; background-color: #ffff00}"); }
        if (s.swr() > 200 or s.alc() > 90) { band_hopping_label.setStyleSheet ("QLabel{color: #ffffff; background-color: #ff0000}"); }
        ...
        if (s.swr() < 1000) {
          if (s.alc() > 0) {
            band_hopping_label.setText(QString {"SWR: %1; ALC: %0%"}.arg(s.alc(), 0, 'f', 0).arg(s.swr() / 100., 0, 'f', 2));
          } else {
            band_hopping_label.setText(QString {"SWR: %1"}.arg (s.swr() / 100., 0, 'f', 2));
          }
        } else {
          if (s.alc() > 0) {
            band_hopping_label.setText(QString {"SWR: %1; ALC: %0%"}.arg(s.alc(), 0, 'f', 0).arg(s.swr() / 100., 0, 'f', 1));
          } else {
            ...
            if (m_config.detailed_blank()) {
              if (m_config.DXCC()) {
                ui->decodedTextBrowser->insertLineSpacer ("------------ " +
                    m_dateTimeSeqStart.toString("yyyy-MM-dd - hh:mm:ss' UTC - '") + band + " - " + m_mode + " --------------");
              } else {
                ui->decodedTextBrowser->insertLineSpacer ("------------ " +
                    m_dateTimeSeqStart.toString("yyyy-MM-dd - hh:mm:ss' UTC - '") + band + " - " + m_mode);
              }
...
void MainWindow::on_cbCQonly_toggled(bool) {
  ...
  to_jt9(m_ihsym, 1, -1);              //Send m_ihsym to jt9[.exe] and start decoding
  decodeBusy(true);
  if (DXcall_only && !not_now) {       //Reset DXcall_only
    DXcall_only = false;
    ui->cbCQonly->setChecked(false);
  }
}
...
void MainWindow::selectHound(QString line, bool bTopQueue) {
  ...
  */
  ...
  ui->houndQueueTextBrowser->insertText(bTopQueue ? t1 + "\n" : t1, QColor{}, QColor{}, houndCall, "",
                                        bTopQueue ? QTextCursor::Start : QTextCursor::End);
...
void MainWindow::foxTxSequencer() {
  ...
  */
  ...
  if (!m_config.superFox() && m_config.OTPEnabled() && (islot < m_Nslots) &&
      (m_tFoxTxSinceOTP >= m_config.OTPinterval())) {
...
void MainWindow::check_button_color() {
  ...
  if ((((m_mode.startsWith ("FT") or m_mode == "Q65" or m_mode == "FST4" or m_mode == "MSK144" or m_mode == "JT65" or
         m_mode == "JT9" or m_mode == "JT4") &&
        m_specOp == SpecOp::NONE && ui->cbAutoSeq->isChecked() && ui->cbAutoSeq->isChecked()
        && m_config.Wait_features_enabled()) or m_specOp == SpecOp::HOUND) && (m_hisCall != "" or (m_mode == "FT8" &&
  ...
  if (DXcall_only) {
    if (!ui->cbCQonly->isVisible()) {
      DXcall_only = false;               // Reset DXcall_only when switching to not supported modes
      ui->cbCQonly->setChecked(false);
    }
    ui->cbCQonly->setStyleSheet("QCheckBox{color: #ffffff; background-color: #ff0000}");
    ui->cbCQonly->setText("DX Call");
  } else {
    ui->cbCQonly->setStyleSheet("");
