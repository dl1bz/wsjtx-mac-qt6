...
public:
  ...
  // waterfall top; Bottom = stack-up from the divider so fresh signals
  // at the top of the waterfall remain visible).
...
signals:
  ...
  // Emitted when the user left-clicks directly on a rendered callsign
  // overlay rect. MainWindow uses it to populate the DX Call field
  // and flip Tx Even/1st to the opposite of the decoded period.
...
private:
  ...
  // Overlay anchor. Top = legacy stacking down from waterfall top.
  ...
  // Hit-test entries rebuilt every time paintDecodeLabels() runs. Each
  // entry is the rendered rect of one callsign label plus the callsign
  // and audio-offset frequency that drew it. mouseReleaseEvent walks
  // this list for plain left-clicks to detect "user clicked on the
  // callsign text" vs. "user clicked on bare waterfall".
  struct ClickableLabel {
    QRect rect;
    QString callsign;
    QString grid;         // copied from DecodeLabel.grid — may be empty
    int freq_hz;
    bool even_period;     // copied from the decode that drew this label
  };
  QList<ClickableLabel> m_clickableLabels;
