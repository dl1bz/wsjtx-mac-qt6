...
void DisplayText::displayDecodedText(DecodedText const& decodedText, QString const& myCall,
  ...
  m_CQPriority = "";
  if (CQcall || (is_73 && m_config->highlight_73()) || (mode == "FT4" && m_config->highlight_73() &&
  ...
  if (myCall.size()) {
    ...
    if ((decodedText.clean_string() + " ").contains(QRegularExpression {regexp})) {
      ...
      if ((tw.size() > 0 && tw[0].contains(Radio::base_callsign(myCall))) or
          decodedText.clean_string().contains("; " + myCall)) {
      ...
      if (!displayDXCCEntity) {
        ...
        if (m_bPrincipalPrefix) {
          if (message.length() < (49 + m_config->align_steps() + m_config->align_steps2())) {
            message = leftJustifyAppendage((message + (space.repeated(30))).left(48 + m_config->align_steps() +
                                           m_config->align_steps2()), "[" + distance + "]");
          } else {
            message = leftJustifyAppendage(message, " [" + distance + "]");
          }
        } else {
          if (message.length() < 59 + m_config->align_steps() + m_config->align_steps2()) {
            message = leftJustifyAppendage((message + (space.repeated(40))).left(59 + m_config->align_steps() +
                                           m_config->align_steps2()), "[" + distance + "]");
          } else {
            message = leftJustifyAppendage(message, "[" + distance + "]");
          }
  ...
  if (mode == "MSK144") {
    if ((m_config->alert_Enabled()) && ((m_config->alert_DXCC()) || (m_config->alert_DXCCOB()) ||
                                        (m_config->alert_Grid()) ||
                                        (m_config->alert_GridOB()) || (m_config->alert_Continent()) || (m_config->alert_ContinentOB()) ||
                                        (m_config->alert_CQZ()) ||
