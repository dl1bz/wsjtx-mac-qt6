...
void Astro::read_settings() {
  ...
  case 0:
    ui_->rbNoDoppler->setChecked(true);
    break;
  case 1:
    ui_->rbFullTrack->setChecked(true);
    break;
  case 2:
    ui_->rbConstFreqOnMoon->setChecked(true);
    break;
  case 3:
    ui_->rbOwnEcho->setChecked(true);
    break;
  case 4:
    ui_->rbOnDxEcho->setChecked(true);
    break;
  case 5:
    ui_->rbCallDx->setChecked(true);
    break;
  case 10:
    ui_->rbNoDoppler->setChecked(true);
    break;
...
auto Astro::astroUpdate(QDateTime const& t, QString const& mygrid, QString const& hisgrid, Frequency freq,
  ...
  if (ui_->cbDopplerTracking->isChecked()) {
    ...
    case 10:
      ...
      break;
    case 1:
      correction.tx = -correction.rx + ibShift;
      break;
    ...
    if (no_tx_QSY && 3 != m_DopplerMethod && 0 != m_DopplerMethod) {
      ...
      case 5:
        offset = m_dop00 - ibShift;// version for _7// maybe +?
        break;
      default:
        offset = -ibShift;// version for _7// maybe +?
        break;
...
void Astro::nominal_frequency(Frequency rx, Frequency tx) {
  if (!ui_->cbLockSkedFreq->isChecked() or astroStart or
