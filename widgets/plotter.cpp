...
void CPlotter::paintDecodeLabels(QPainter& painter) {
  // Rebuild hit-test list every paint — keeps it in sync with what's
  // actually visible (fade-out, scroll, dropped rows all handled).
  ...
  // Upper-waterfall geometry. Top edge = y=30 (below the 30-px freq
  // scale). Bottom edge = m_h1 (resize event computes m_h1 = m_h-m_h2,
  // and the 2D-spectrum / divider starts there). Same anchor convention
  // as QMAP/MAP65 — Top stacks down from y=30; Bottom stacks up from
  // m_h1 so labels sit just above the divider.
  constexpr int waterfall_top_y = 30;
  const int waterfall_bottom_y  = m_h1;
  ...
  for (const auto& l : sorted) {
    ...
    // Top-anchor: row 0 at top of upper waterfall, stacks down.
    // Bottom-anchor: row 0 just above divider, stacks up.
    ...
    // Tick points toward the visible signal trace: down (toward the
    // divider) when stacked from top; up (toward the freq scale)
    // when stacked from bottom.
    const int tick_y0 = stack_from_bottom ? y_top     : y_top + row_height;
    const int tick_y1 = stack_from_bottom ? y_top - 3 : y_top + row_height + 3;
    ...
    if (l.is_active) {
      ...
      text_col = l.is_even_period ? QColor(255,  140,   0)  // #ff8c00 — orange
                 : QColor(180,   0,   0);
    } else {
    ...
    // Record the rendered rect for mouse hit-testing. Stored in draw
    // order so mouseReleaseEvent can walk it from the end and pick the
    // top-most label when two rects overlap.
    m_clickableLabels.append(
    {rect, l.callsign, l.grid, static_cast<int>(l.freq_hz + 0.5), l.is_even_period});
  }
}
...
void CPlotter::DrawOverlay() {                 //DrawOverlay()
  ...
  if (m_mode.startsWith("FT") or m_mode.startsWith("JT") or m_mode == "Q65" or m_mode.startsWith("FST4")) {
    ...
    if (m_bars) {
      ...
      overPainter.drawLine(x1, 0, x1, m_h);
      overPainter.drawLine(x2, 0, x2, m_h);
    }
  }
  // red bars
  if (m_mode.startsWith("FT") or m_mode.startsWith("JT") or m_mode == "Q65" or m_mode.startsWith("FST4")
...
void CPlotter::mouseReleaseEvent(QMouseEvent * event) {
  ...
  if (leftbutton) {
    ...
    // Plain left-click directly on a callsign overlay rect:
    //  - populate the DX Call field via callsignClicked
    //  - move the RX bars to that signal's audio offset
    //  - leave TX alone (matches plain-click semantics elsewhere in this
    //    handler and respects Hold Tx Freq implicitly)
    // Modifier-clicks fall through to the existing tune-and-split logic.
    if (!shift && !ctrl && !m_clickableLabels.isEmpty()) {
      const QPoint p = event->pos();
      for (int i = m_clickableLabels.size() - 1; i >= 0; --i) {
        const auto& hit = m_clickableLabels[i];
        if (hit.rect.contains(p)) {
          emit callsignClicked(hit.callsign, hit.freq_hz, hit.even_period, hit.grid);
          emit setFreq1(hit.freq_hz, m_txFreq);
