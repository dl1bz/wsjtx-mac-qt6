...
private slots:
  ...
  void overlayCallsignClicked(QString call, int freq_hz, bool decoded_even_period, QString grid);
...
private:
  ...
  bool    m_discard_decoded_hounds_this_cycle =
          false;   // if something changes, like frequency, discard decoded messages that may be in-flight.
  ...
  void replyToCQ(QTime, qint32 snr, float delta_time, quint32 delta_frequency, QString const& mode,
  ...
  QString userAgent();
  void handleVerifyMsg(int status, QDateTime ts, QString callsign, QString code, unsigned int hz,
