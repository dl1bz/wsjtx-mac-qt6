...
public:
  ...
  void check_connection() {
    if (!socket_
        ...
        || (socket_->socketType() != (config_->psk_reporter_tcpip() ? QAbstractSocket::TcpSocket :
  ...
  void reconnect() {
    ...
    connect(socket_.data(), QOverload<QAbstractSocket::SocketError>::of(&QAbstractSocket::error), this,
            &PSKReporter::impl::handle_socket_error);
#else
    connect(socket_.data(), static_cast<void (QAbstractSocket::*)(QAbstractSocket::SocketError)> (&QAbstractSocket::error),
            this, &PSKReporter::impl::handle_socket_error);
#endif
...
bool PSKReporter::addRemoteStation(QString const& call, QString const& grid, Radio::Frequency freq
  ...
  LOG_LOG_LOCATION(m_->logger_, trace,
                   "call: " << call << " grid: " << grid << " freq: " << freq << " mode: " << mode << " snr: " << snr);
  m_->check_connection();
