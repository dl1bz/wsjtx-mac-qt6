...
void FileDownload::configure(QNetworkAccessManager *network_manager, const QString &source_url,
                             const QString &destination_path, const QString &user_agent) {
...
void FileDownload::downloadComplete(QNetworkReply *data) {
  ...
  QUrl r = request_.url();
  LOG_INFO(QString{"FileDownload [%1]: finished %2 of %3 -> %4 (%5)"}.arg(user_agent_).arg(data->operation()).arg(
                   source_url_).arg(destination_filename_).arg(r.url()));
...
#endif
  LOG_INFO(QString {"FileDownload [%1]: Starting download of %2 to %3"}.arg(user_agent_).arg(source_url_).arg(
                   destination_filename_));
...
#else
  QObject::connect(reply_, QOverload<QNetworkReply::NetworkError>::of(&QNetworkReply::error), this,
...
void FileDownload::downloadProgress(qint64 received, qint64 total) {
  LOG_DEBUG(QString{"FileDownload: [%1] Progress %2 from %3, total %4, so far %5"}.arg(user_agent_).arg(
                    destination_filename_).arg(source_url_).arg(total).arg(received));
