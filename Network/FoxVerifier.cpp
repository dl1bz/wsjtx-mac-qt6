...
FoxVerifier::FoxVerifier(QString user_agent, QNetworkAccessManager *manager, QString base_url, QString callsign,
                         QDateTime timestamp, QString code, unsigned int hz = 750) : QObject(nullptr) {
  manager_ = manager;
  finished_ = false;
  errored_ = false;
  callsign_ = callsign;
  ...
  QString url = QString("%1/check/").arg(base_url) + encodedCall + QString("/%1/%2.text").arg(timestamp.toString(
                        Qt::ISODate)).arg(code);
  ...
  if (q_url_.isValid()) {
    ...
    //    request_.setAttribute(QNetworkRequest::FollowRedirectsAttribute, true);  // does not work with Qt6
...
void FoxVerifier::errorOccurred(QNetworkReply::NetworkError code) {
  ...
  if (reply_->error() != QNetworkReply::NoError) {
    LOG_INFO(QString("FoxVerifier: errorOccurred status %1 error [%2][%3] isFinished %4 isrunning %5 code %6").arg(
                     status).arg(
...
void FoxVerifier::httpFinished() {
  ...
  QString reason = reply_->attribute(QNetworkRequest::HttpReasonPhraseAttribute).toString();
  if (reply_->error() != QNetworkReply::NoError) {
    LOG_INFO(QString("FoxVerifier: httpFinished error:[%1 - %2] msg:[%3]").arg(status).arg(reason).arg(
  ...
  LOG_INFO(QString("FoxVerifier: httpFinished status:[%1 - %2] body:[%3] ").arg(status).arg(reason).arg(
                   return_value).toStdString());
  finished_ = true;
  reply_->deleteLater();
...
QString FoxVerifier::formatDecodeMessage(QDateTime ts, QString callsign, unsigned int hz_,
    QString const& verify_message) {
  ...
  } else if (verify_message.endsWith(" INVALID")) {
