  ...
  try {
    ...
    if (requested_.online()) {
      ...
      if (requested_.tx_audio() != s.tx_audio()) {
        if (s.tx_audio()) {
          do_modulator_start(s.jtmode(), s.symbolslength(), s.framespersymbol(), s.trfrequency(), s.tonespacing(),
                             s.synchronize(), s.fastmode(), s.dbsnr(), s.trperiod());
...
void TransceiverBase::update_alc(qreal p) {
  CAT_TRACE("alc: " << p);
  actual_.alc(p);
}
