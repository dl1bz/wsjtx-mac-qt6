...
int HamlibTransceiver::impl::debug_callback(enum rig_debug_level_e level, rig_ptr_t arg, char const * format,
    va_list ap) {
...
std::tuple<vfo_t, vfo_t> HamlibTransceiver::impl::get_vfos(bool for_split) const {
  ...
  } else if (!for_split && set_vfo_works_ && rig_get_function_ptr(model_, RIG_FUNCTION_SET_VFO) &&
             rig_get_function_ptr(model_, RIG_FUNCTION_SET_SPLIT_VFO)) {
...
QByteArray HamlibTransceiver::impl::get_conf(char const * item) {
  ...
  if (RIG_CONF_END != token) { // only get if valid for rig model
    error_check(rig_get_conf2(rig_.data(), token, value.data(), value.length()),
                tr("getting a configuration item"));     //changed to conf2, added parameter for buffer length
  }
  return value;
}
  ...
  if (!m_->is_dummy_) {
    ...
    case RIG_PORT_SERIAL:
      ...
      case TransceiverFactory::handshake_none:
        m_->set_conf("serial_handshake", "None");
        break;
      case TransceiverFactory::handshake_XonXoff:
        m_->set_conf("serial_handshake", "XONXOFF");
        break;
      case TransceiverFactory::handshake_hardware:
        m_->set_conf("serial_handshake", "Hardware");
        break;
      default:
        break;
      }
...
int HamlibTransceiver::do_start() {
  ...
  do_pwr_ &= (!m_->is_dummy_ && rig_get_function_ptr(m_->model_, RIG_FUNCTION_GET_LEVEL) &&
              (rig_get_caps_int(m_->model_, RIG_CAPS_HAS_GET_LEVEL) & RIG_LEVEL_RFPOWER_METER_WATTS) ==
  ...
  do_alc_ &= (!m_->is_dummy_ && rig_get_function_ptr(m_->model_, RIG_FUNCTION_GET_LEVEL) &&
              (rig_get_caps_int(m_->model_, RIG_CAPS_HAS_GET_LEVEL) & RIG_LEVEL_ALC) == RIG_LEVEL_ALC);
    ...
    if (m_->freq_query_works_ && m_->mode_query_works_
      ...
      if (m_->set_vfo_works_) {
        ...
        if (!rig_get_function_ptr(m_->model_, RIG_FUNCTION_SET_VFO)) {
          ...
          m_->error_check(rig_set_vfo(m_->rig_.data(), m_->rig_->state.vfo_list & RIG_VFO_A ? RIG_VFO_A : RIG_VFO_MAIN),
                          tr("setting current VFO"));
        }
      ...
      if (m_->get_vfo_works_ && rig_get_function_ptr(m_->model_, RIG_FUNCTION_GET_VFO)) {
        m_->error_check(rig_get_vfo(m_->rig_.data(), &v),
                        tr("getting current VFO"));     // has side effect of establishing current VFO inside hamlib
...
void HamlibTransceiver::do_frequency(Frequency f, MODE m, bool no_ignore) {
  ...
  if (!state().ptt() || !state().split() || !m_->one_VFO_ || no_ignore) {
    ...
    if (m_->mode_query_works_ && UNK != m) {
      ...
      m_->error_check(rig_get_mode(m_->rig_.data(), target_vfo, &current_mode, &current_width),
      ...
      if (new_mode != current_mode) {
        ...
        m_->error_check(rig_set_mode(m_->rig_.data(), target_vfo, new_mode, RIG_PASSBAND_NOCHANGE),
    ...
    if (tx) {
      ...
      if (state().ptt() && m_->one_VFO_) {
        ...
        if (UNK != mode && m_->mode_query_works_) {
          ...
          m_->error_check(rig_get_mode(m_->rig_.data(), RIG_VFO_CURR, &current_mode, &current_width),
                          tr("getting current VFO mode"));
          CAT_TRACE("rig_get_mode mode=" << rig_strrmode(current_mode) << " bw=" << current_width);
          if (new_mode != current_mode) {
            CAT_TRACE("rig_set_mode mode=" << rig_strrmode(new_mode));
            m_->error_check(rig_set_mode(m_->rig_.data(), RIG_VFO_CURR, new_mode, RIG_PASSBAND_NOCHANGE),
                            tr("setting current VFO mode"));
          }
        }
        ...
        if (UNK != mode) {
          ...
          m_->error_check(rig_set_split_freq_mode(m_->rig_.data(), RIG_VFO_CURR, tx, new_mode, RIG_PASSBAND_NOCHANGE),
...
void HamlibTransceiver::do_mode(MODE mode) {
  ...
  if (state().ptt() && state().split() && m_->one_VFO_) {
    ...
    m_->error_check(rig_get_split_mode(m_->rig_.data(), RIG_VFO_CURR, &current_mode, &current_width),
    ...
    if (new_mode != current_mode) {
      ...
      hamlib_tx_vfo_fixup fixup(m_->rig_.data(), tx_vfo);
      m_->error_check(rig_set_split_mode(m_->rig_.data(), RIG_VFO_CURR, new_mode, RIG_PASSBAND_NOCHANGE),
...
void HamlibTransceiver::do_poll() {
  ...
  if (ptt_on_) {
    ...
    int i_alc;
    if (do_alc_) {
      rc = rig_get_level(m_->rig_.data(), RIG_VFO_CURR, RIG_LEVEL_ALC, &strength);
      f_alc = strength.f;
      if (RIG_OK == rc && ptt_on_) {
        strength.f = strength.f * 100;
        //i_alc = static_cast<int>(strength.f);
        i_alc = trunc(strength.f);
        update_alc(i_alc);
      } else {
