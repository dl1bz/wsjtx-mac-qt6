...
void HRDTransceiver::register_transceivers(logger_type *,
    ...
    unsigned id) {
  (*registry)[HRD_transceiver_name] = TransceiverFactory::Capabilities(id, TransceiverFactory::Capabilities::network,
                                      true, true /* maybe */);
}
...
int HRDTransceiver::do_start() {
  ...
  if ((data_mode_dropdown_ = find_dropdown(QRegularExpression("^(Data)$"))) >= 0) {
    data_mode_dropdown_selection_on_ = find_dropdown_selection(data_mode_dropdown_,
...
void HRDTransceiver::map_modes(int dropdown, ModeMap *map) {
  ...
  map->push_back(std::forward_as_tuple(CW, find_dropdown_selection(dropdown,
                                       QRegularExpression("^(CW|CW\\(N\\)|CW-LSB|CWL)$"))));
  map->push_back(std::forward_as_tuple(CW_R, find_dropdown_selection(dropdown,
                                       QRegularExpression("^(CW-R|CW-R\\(N\\)|CW|CW-USB|CWU)$"))));
  ...
  map->push_back(std::forward_as_tuple(DIG_U, find_dropdown_selection(dropdown,
                                       QRegularExpression("^(DIG|DIGU|DATA-U|PKT-U|DATA|AFSK|USER-U|USB)$"))));
  map->push_back(std::forward_as_tuple(DIG_L, find_dropdown_selection(dropdown,
                                       QRegularExpression("^(DIG|DIGL|DATA-L|PKT-L|DATA-R|USER-L|LSB)$"))));
  map->push_back(std::forward_as_tuple(FSK, find_dropdown_selection(dropdown,
                                       QRegularExpression("^(DIG|FSK|RTTY|RTTY-LSB)$"))));
  map->push_back(std::forward_as_tuple(FSK_R, find_dropdown_selection(dropdown,
  ...
  map->push_back(std::forward_as_tuple(FM, find_dropdown_selection(dropdown,
                                       QRegularExpression("^(FM|FM\\(N\\)|FM-N|WFM)$"))));
  map->push_back(std::forward_as_tuple(DIG_FM, find_dropdown_selection(dropdown,
                                       QRegularExpression("^(PKT-FM|PKT|DATA\\(FM\\)|FM)$"))));
  ...
  std::for_each(map->begin(), map->end(), [this, dropdown](ModeMap::value_type const & item) {
    ...
    CAT_TRACE('\t' << std::get<0> (item) << "<->" << (rhs.size() ? dropdowns_[dropdown_names_[dropdown]][rhs.front()] :
              "None"));
  });
}
...
void HRDTransceiver::set_dropdown(int dd, int value) {
  ...
  if (value >= 0) {
    send_simple_command("set dropdown " + dd_name.replace(' ',
                        '~') + ' ' + dropdowns_.value(dd_name).value(value).replace(' ', '~') + ' ' + QString::number(value));
  } else {
...
void HRDTransceiver::do_tx_frequency(Frequency tx, MODE mode, bool /*no_ignore*/) {
  ...
  if (split_mode_button_ >= 0) {
    ...
    set_dropdown(split_mode_dropdown_, split ? split_mode_dropdown_selection_on_.front() :
