...
void MainWindow::guiUpdate() {
  ...
  if ((iptt == 1 && iptt0 == 0) || m_restart) {
    ...
    if (m_restart) {
      ...
      qDebug() << "MainWindow::guiUpdate 1 File open result:" << f.open(QFileDevice::WriteOnly | QFileDevice::Text |
               QFileDevice::Append);
      QTextStream out(&f);
  ...
  if (nc1 == 0) {
    ...
    t = t.left(11);
    QFile f("map65_tx.log");
    qDebug() << "MainWindow::guiUpdate 2 File open result:" << f.open(QFileDevice::WriteOnly | QFileDevice::Text |
             QFileDevice::Append);
    QTextStream out(&f);
    out << QDateTime::currentDateTimeUtc().toString("yyyy-MMM-dd hh:mm")
        << t << QString::fromLatin1(msgsent)
...
void MainWindow::on_addButton_clicked() {                     //Add button
  ...
  if (m_call3Modified) {
    ...
    // symlinks
    qDebug() << "MainWindow::on_addButton_clicked File open result:" << f1.open(QFileDevice::WriteOnly |
             QFileDevice::Text); // truncates
    f2.seek(0);
